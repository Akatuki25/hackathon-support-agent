 ...
 * @returns 生成された環境構築情報
 */
export const generateEnvSetup = async (
  projectId: string,
): Promise<EnvSetupResponse> => {
 ...
 */
export const regenerateEnvSetup = async (
  projectId: string,
): Promise<EnvSetupResponse> => {
  const response = await axios.post<EnvSetupResponse>(
 ...
 */
export const getEnvSetupOrNull = async (
  projectId: string,
): Promise<EnvGetResponse | null> => {
  try {
 ...
 * @returns 環境構築情報
 */
export const getOrGenerateEnvSetup = async (
  projectId: string,
): Promise<EnvSetupResponse | EnvGetResponse> => {
  // まず既存データを確認
