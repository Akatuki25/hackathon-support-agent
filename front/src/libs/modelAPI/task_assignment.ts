...
import { TaskAssignmentType, TaskAssignmentResponseType, TaskAssignmentPatch } from '@/types/modelTypes';
...
export const useTaskAssignments = () => {
  const { data, error } = useSWR<TaskAssignmentType[]>(`${API_URL}/task_assignments`, fetcher);
  return {
...
export const useTaskAssignmentsByProjectMemberId = (projectMemberId?: string) => {
  const { data, error } = useSWR<TaskAssignmentType[]>(
    projectMemberId ? `${API_URL}/task_assignment/project_member/${projectMemberId}` : null,
    fetcher
  );
  return {
    taskAssignments: data,
...
export const postTaskAssignment = async (
  ...
  mutate(`${API_URL}/task_assignment/project_member/${taskAssignment.project_member_id}`); // Revalidate by project member ID
...
export const putTaskAssignment = async (
  ...
  const response = await fetch(`${API_URL}/task_assignment/${taskAssignmentId}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(taskAssignment),
  });
...
export const patchTaskAssignment = async (
  ...
  mutate(`${API_URL}/task_assignment/project_member/${data.task_assignment_id}`); // Revalidate by project member ID (using returned task_assignment_id)
  return data.message;
};
