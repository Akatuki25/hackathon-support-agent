...
// --- PUT QA ---
export const putQA = async (qaId: string, qa: Omit<QAType, 'qa_id' | 'created_at'>): Promise<string> => {
  const response = await axios.put<QAResponseType>(`${API_URL}/qa/${qaId}`, qa);
  return response.data.message;
};

// --- PATCH QA ---
export const patchQA = async (qaId: string, qaPatch: QAPatch): Promise<string> => {
  const response = await axios.patch<{ message: string }>(`${API_URL}/qa/${qaId}`, qaPatch);
  return response.data.message;
};
