...
export const getMemberById = async (memberId: string): Promise<MemberType> => {
  const response = await axios.get<MemberType>(
    `${API_URL}/member/id/${memberId}`,
  );
  return response.data;
};

// --- GET Member by GitHub Name ---
export const getMemberByGithubName = async (
  githubName: string,
): Promise<MemberType> => {
  const response = await axios.get<MemberType>(
    `${API_URL}/member/github/${githubName}`,
  );
  return response.data;
};
...
export const postMember = async (member: MemberType): Promise<string> => {
  const response = await axios.post<MemberResponseType>(
    `${API_URL}/member`,
    member,
  );
...
// --- PUT Member by ID ---
export const putMemberById = async (
  memberId: string,
  member: MemberType,
): Promise<string> => {
  const response = await axios.put<MemberResponseType>(
    `${API_URL}/member/id/${memberId}`,
    member,
  );
...
export const putMemberByGithubName = async (
  githubName: string,
  member: MemberType,
): Promise<string> => {
  const response = await axios.put<MemberResponseType>(
...
// --- PATCH Member by ID ---
export const patchMemberById = async (
  memberId: string,
  memberPatch: MemberPatch,
): Promise<string> => {
  const response = await axios.patch<MemberResponseType>(
    `${API_URL}/member/id/${memberId}`,
    memberPatch,
  );
  return response.data.message;
...
export const deleteMemberByGithubName = async (
  githubName: string,
): Promise<{ message: string }> => {
