...
import { Github, Linkedin, Twitter, Mail, Code, Zap, Users, Target, Trophy, Clock, MessageCircle, Lightbulb } from 'lucide-react';
...
export default function HackathonSupportAgentLandingPage() {
  ...
  const members = [
    {
      name: "Alex Chen",
      role: "AI Engineer",
      description: "機械学習とNLP分野で8年の経験。インテリジェントなアシスタントシステムの構築が専門。",
    ...
    },
    {
      ...
      description: "ユーザー体験設計とデザインシステムのエキスパート。ハッカソン向けツールのUI/UX設計が得意。",
    ...
    },
    {
      ...
      description: "リアルタイムシステムとAPIアーキテクチャのスペシャリスト。高可用性システムの構築経験豊富。",
    ...
    },
    {
      ...
      description: "50以上のハッカソンに参加・運営経験。チーム戦略とプロジェクト管理のコンサルティングが専門。",
  ...
  const stats = [
    { number: "500+", label: "サポートしたハッカソン", code: "HACKATHONS_SUPPORTED" },
    ...
    <div className={`min-h-screen transition-all duration-300 ${
      ...
      <div className="relative z-10 container mx-auto px-6 pt-32 pb-12">
        ...
        <div className="text-center mb-20">
          <div className={`inline-block px-4 py-2 rounded-lg font-mono text-sm mb-6 ${
            darkMode 
          ...
          <h1 className={`text-4xl md:text-6xl font-bold mb-6 font-mono tracking-wider ${
            darkMode 
          ...
          <p className={`text-lg md:text-xl mb-8 ${darkMode ? 'text-gray-300' : 'text-gray-600'} font-mono max-w-3xl mx-auto`}>
            // AIの力でハッカソンを勝利に導く、あなたの最強パートナー
          </p>
          
          {/* Cyber divider */}
          <div className="flex items-center justify-center mb-8">
            <div className={`h-px w-12 ${darkMode ? 'bg-cyan-500' : 'bg-purple-500'}`}></div>
            <div className={`mx-4 w-2 h-2 border ${darkMode ? 'border-cyan-500' : 'border-purple-500'} rotate-45`}></div>
            <div className={`h-px w-12 ${darkMode ? 'bg-pink-500' : 'bg-blue-500'}`}></div>
          </div>
          
          {/* CTA Button */}
          <button className={`group relative px-8 py-4 rounded-lg font-mono font-bold text-lg tracking-wider transition-all duration-300 overflow-hidden ${
            darkMode 
            ...
            <div className={`absolute inset-0 ${
              darkMode ? 'bg-gradient-to-r from-transparent via-cyan-400/20 to-transparent' : 'bg-gradient-to-r from-transparent via-purple-400/20 to-transparent'
            ...
            <div className={`absolute top-1 right-1 w-2 h-2 border-r border-t ${
              darkMode ? 'border-cyan-400/50' : 'border-purple-400/50'
            } opacity-0 group-hover:opacity-100 transition-opacity`}></div>
          </button>
        </div>
            ...
            {stats.map((stat, index) => (
              ...
              >
                ...
                <div className="text-center relative">
                  ...
                  <div className={`text-xs mb-1 font-mono ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    // {stat.code}
                  </div>
        ...
        {/* Features Section */}
        <div className="mb-20">
          <h2 className={`text-2xl md:text-3xl font-bold text-center mb-12 font-mono tracking-wider ${
            darkMode ? 'text-cyan-400' : 'text-purple-600'
          }`}>
            // SYSTEM_FEATURES
          </h2>
            ...
            {features.map((feature, index) => (
              ...
              >
                ...
                <div className={`absolute top-0 left-0 right-0 h-px ${
                  darkMode ? 'bg-gradient-to-r from-transparent via-cyan-400/50 to-transparent' : 'bg-gradient-to-r from-transparent via-purple-400/50 to-transparent'
                } translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000`}></div>
                
                <div className={`${darkMode ? 'text-cyan-400' : 'text-purple-600'} mb-4 relative`}>
                  {feature.icon}
                  {/* Icon brackets */}
                ...
                <h3 className={`text-lg font-semibold mb-2 font-mono tracking-wider ${
                  darkMode ? 'text-white' : 'text-gray-900'
                }`}>
                  {feature.title}
                </h3>
                <p className={`text-xs mb-3 font-mono ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  {feature.code}
                </p>
        ...
        <div className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={index} className={`relative text-center p-6 rounded-lg backdrop-blur-sm border ${
                darkMode 
                  ? 'bg-gray-900/30 border-cyan-500/20' 
                  : 'bg-white/30 border-purple-300/20'
              }`}>
                {/* Step number with cyber styling */}
                <div className={`relative w-16 h-16 mx-auto mb-4 rounded border-2 flex items-center justify-center text-xl font-bold font-mono ${
                  darkMode 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {members.map((member, index) => (
              ...
              >
                ...
                } opacity-0 group-hover:opacity-100 transition-opacity animate-pulse`}></div>
                
                <div className="text-center relative">
                  <div className={`relative w-24 h-24 mx-auto mb-4 rounded border-4 overflow-hidden ${
                  ...
                  <p className={`text-sm mb-1 font-mono ${darkMode ? 'text-cyan-400' : 'text-purple-600'}`}>
                  ...
                  <div className="flex justify-center space-x-3">
                    {member.social.github && (
                      <a href={member.social.github} className={`relative group/icon ${
                        darkMode ? 'text-gray-400 hover:text-cyan-400' : 'text-gray-500 hover:text-purple-600'
                      } transition-colors duration-200`}>
                        <Github className="w-4 h-4" />
                        <div className={`absolute -inset-1 border ${
                          darkMode ? 'border-cyan-400/30' : 'border-purple-400/30'
                        } opacity-0 group-hover/icon:opacity-100 transition-opacity`}></div>
                      </a>
                    )}
                    {member.social.linkedin && (
                      <a href={member.social.linkedin} className={`relative group/icon ${
                        darkMode ? 'text-gray-400 hover:text-cyan-400' : 'text-gray-500 hover:text-purple-600'
                      } transition-colors duration-200`}>
                    ...
                    {member.social.twitter && (
                      <a href={member.social.twitter} className={`relative group/icon ${
                        darkMode ? 'text-gray-400 hover:text-cyan-400' : 'text-gray-500 hover:text-purple-600'
                      } transition-colors duration-200`}>
                    ...
                    {member.social.mail && (
                      <a href={member.social.mail} className={`relative group/icon ${
                        darkMode ? 'text-gray-400 hover:text-cyan-400' : 'text-gray-500 hover:text-purple-600'
                      } transition-colors duration-200`}>
        ...
        {/* CTA Section */}
        <div className={`relative text-center p-12 rounded-lg backdrop-blur-sm border overflow-hidden ${
          darkMode 
          ...
          <div className="relative">
            <div className={`inline-block px-4 py-2 rounded font-mono text-sm mb-4 ${
              darkMode 
            ...
            <h2 className={`text-2xl md:text-3xl font-bold mb-4 font-mono tracking-wider ${
              darkMode ? 'text-white' : 'text-gray-900'
            }`}>
        ...
        <div className="text-center">
          <div className="flex items-center justify-center mb-6">
            <div className={`h-px w-12 ${darkMode ? 'bg-cyan-500' : 'bg-purple-500'}`}></div>
            <div className={`mx-4 text-xs font-mono ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              // END_OF_TRANSMISSION
            </div>
            <div className={`h-px w-12 ${darkMode ? 'bg-pink-500' : 'bg-blue-500'}`}></div>
          </div>
          <p className={`${darkMode ? 'text-gray-400' : 'text-gray-500'} text-sm font-mono`}>
