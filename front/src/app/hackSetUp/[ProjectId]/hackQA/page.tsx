...
export default function HackQA() {
  ...
  const startStreamingGeneration = async () => {
    ...
    try {
      await streamGenerateQuestions(projectId, questionData, {
        onStart: () => {
        ...
        onDone: async (data) => {
          ...
          try {
            const payload = convertStreamingItemsToPayload(
              streamingQAsRef.current,
            );
  ...
  const ensureUserIsProjectMember = async (
    projectId: string,
    githubName: string,
  ) => {
    try {
      const { getProjectMembersByProjectId, postProjectMember } = await import(
        "@/libs/modelAPI/project_member"
      );
  ...
  const handleDeleteQA = async (qaId: string) => {
    try {
      ...
      mutateQAs((currentQAs) => currentQAs?.filter((qa) => qa.qa_id !== qaId), {
        revalidate: false,
      });
    } catch (error) {
  ...
  const shouldShowStreamingData =
    ...
    (streamingStatus === "done" &&
      streamingQAs.length > 0 &&
      (!qas || qas.length === 0));

  const displayQAs = shouldShowStreamingData ? streamingQAsAsQAType : qas || [];
    ...
    <>
      ...
      <main className="relative z-10">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-8">
            <div className="flex items-center justify-center mb-4 mt-5">
              ...
              <h1 className="text-3xl font-bold tracking-wider text-purple-700 dark:text-cyan-400">
                プロジェクト
                <span className="text-blue-600 dark:text-pink-500">_分析</span>
              </h1>
            </div>
          ...
          <div className="backdrop-blur-lg rounded-xl p-8 shadow-xl border transition-all bg-white bg-opacity-70 border-purple-500/30 shadow-purple-300/20 dark:bg-gray-800 dark:bg-opacity-70 dark:border-cyan-500/30 dark:shadow-cyan-500/20">
            <>
              {/* プロジェクトのアイデア表示 */}
              <div className="mb-6">
                <h2 className="text-xl font-medium mb-4 flex items-center text-purple-700 dark:text-cyan-400">
                  <Database
                    size={18}
                ...
                {idea ? (
                  <p className="bg-purple-100 text-gray-800 dark:bg-gray-700 dark:text-cyan-300 p-4 rounded-lg border-l-4 border-blue-500 dark:border-pink-500">
                    {idea}
                  </p>
                ) : (
                  <div className="p-4 rounded-lg border-l-4 animate-pulse bg-purple-100 border-blue-500 dark:bg-gray-700 dark:border-pink-500">
                ...
                <div className="space-y-4">
                  {displayQAs.length > 0 ? (
                    <>
                      ...
                      {displayQAs.map((qa) => (
                        ...
                        >
                          ...
                          <div className="mb-3 flex items-start justify-between">
                            {editingQA?.id === qa.qa_id &&
                            editingQA.field === "question" ? (
                              <input
                                type="text"
                      ...
                      {/* ストリーミング中は新規追加を非表示 */}
                      {streamingStatus === "streaming" ||
                      streamingStatus === "saving" ? null : showAddQA ? (
                        <div className="p-5 rounded-lg border-2 border-dashed transition-all bg-purple-50/50 border-purple-400/50 dark:bg-gray-700/30 dark:border-cyan-500/50">
                          <h3 className="text-lg font-semibold mb-4 text-purple-700 dark:text-cyan-400">
                            新しいQ&Aを追加
                          </h3>
                          <div className="space-y-4">
                            <div>
                              <label className="block mb-2 text-sm font-medium text-purple-600 dark:text-cyan-300">
                                質問:
                              </label>
                              <input
                                type="text"
                        ...
                        >
                          <div className="flex items-center justify-center">
                            ...
                            />
                            <span className="font-medium text-purple-600 dark:text-cyan-400">
                              新しい質問を追加
                            </span>
                          </div>
                        </button>
