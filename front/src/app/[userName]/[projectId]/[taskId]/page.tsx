...
export default function TaskHandsOnPage() {
  ...
  const [handsOnState, setHandsOnState] = useState<HandsOnState>({
    status: "idle",
  });
  ...
  const pageBackgroundClass =
    "min-h-screen bg-gray-100 p-6 text-gray-800 dark:bg-gradient-to-br dark:from-slate-950 dark:via-indigo-950 dark:to-slate-900 dark:text-slate-100";

  const panelClass =
    "mx-auto flex max-w-4xl flex-col gap-6 rounded-lg border border-slate-100 bg-white p-6 shadow-sm dark:border-cyan-500/20 dark:bg-slate-950/60 dark:shadow-[0_0_32px_rgba(6,182,212,0.18)] dark:backdrop-blur";

  const sectionTitleClass =
    "text-lg font-semibold text-blue-700 dark:text-cyan-200";
  const sectionBodyClass = "text-sm text-gray-700 dark:text-slate-200";
  const badgeClass =
    "rounded border border-blue-200 bg-blue-50 px-2 py-1 text-xs text-blue-600 dark:border-cyan-500/40 dark:bg-slate-900/70 dark:text-cyan-200";
  const backLinkClass =
    "inline-flex items-center gap-1 text-sm text-blue-600 underline-offset-4 hover:text-blue-500 hover:underline dark:text-cyan-200 dark:hover:text-cyan-100";

  // Style classes for hands-on content (combined light dark: dark)
  const contentBgClass =
    "bg-gray-50 border border-gray-200 dark:bg-slate-900/80 dark:border-cyan-500/20";
  const textClass = "text-gray-700 dark:text-slate-200";
  const headingClass = "text-blue-700 font-semibold dark:text-cyan-300";
  const codeBlockClass =
    "bg-gray-800 text-green-300 dark:bg-black/60 dark:text-green-200";
          ...
          <div className="space-y-6">
            ...
            {handsOn.prerequisites && (
              <div className={`${contentBgClass} rounded-lg p-4 space-y-2`}>
                ...
                <p className={`${textClass} text-sm whitespace-pre-wrap`}>
                  {handsOn.prerequisites}
                </p>
              </div>
            )}
            ...
            {handsOn.target_files && handsOn.target_files.length > 0 && (
              <div className={`${contentBgClass} rounded-lg p-4 space-y-2`}>
                ...
                <ul
                  className={`${listClass} text-sm space-y-1 list-disc list-inside`}
                >
                  {handsOn.target_files.map((file, index) => (
                    <li key={index}>
                      <code className="font-mono text-xs">{file.path}</code> -{" "}
                      {file.description} ({file.action})
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {handsOn.implementation_steps && (
              <div className={`${contentBgClass} rounded-lg p-4 space-y-2`}>
                ...
                <div
                  className={`${textClass} text-sm prose prose-sm max-w-none whitespace-pre-wrap`}
                >
                  {handsOn.implementation_steps}
                </div>
              </div>
            ...
            {handsOn.code_examples && handsOn.code_examples.length > 0 && (
              <div className="space-y-4">
                ...
                {handsOn.code_examples.map((example, index) => (
                  <div
                    key={index}
                    className={`${contentBgClass} rounded-lg p-4 space-y-2`}
                  >
                    ...
                    <pre
                      className={`${codeBlockClass} rounded p-3 text-xs overflow-x-auto`}
                    >
                      <code>{example.code}</code>
                    </pre>
                    {example.explanation && (
                      <p className={`${listClass} text-xs italic`}>
                        {example.explanation}
                      </p>
                    )}
                  </div>
            ...
            {/* Implementation Tips */}
            {handsOn.implementation_tips &&
              handsOn.implementation_tips.length > 0 && (
                <div className={`${contentBgClass} rounded-lg p-4 space-y-3`}>
                ...
                <ul className={`${listClass} text-sm space-y-1`}>
                  {handsOn.references.map((ref, index) => (
                    <li key={index}>
                      ...
                      {ref.type && (
                        <span className="text-xs ml-1">({ref.type})</span>
                      )}
                    </li>
                  ))}
                </ul>
  ...
  const taskContent = (() => {
    ...
    return (
      <div className="space-y-2">
        <p className={sectionBodyClass}>
          <span className={badgeClass}>ID</span>{" "}
          <span className="font-mono">{task.task_id}</span>
        </p>
        <p className={`${sectionBodyClass} text-base font-semibold`}>
          {task.title}
        </p>
        {task.description && (
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          ...
          {task.status && (
            <span className={badgeClass}>ステータス: {task.status}</span>
          )}
          {task.assignee && (
            <span className={badgeClass}>担当: {task.assignee}</span>
          )}
        </div>
      </div>
    );
    ...
    <div className={pageBackgroundClass}>
      <div className={panelClass}>
        <header className="flex flex-col gap-2">
          ...
          {projectId && userName && (
            <Link
              href={`/${userName}/${projectId}/kanban`}
