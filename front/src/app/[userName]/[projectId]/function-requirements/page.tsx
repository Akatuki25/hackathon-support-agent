...
export default function FunctionRequirementsPage() {
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR(projectId ? `project-document-${projectId}` : null, () =>
        ...
        <div className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 backdrop-blur-xl rounded-2xl border-2 border-amber-500/40 shadow-2xl p-8">
          {isLoading ? (
            ...
            <div className="prose prose-invert max-w-none text-gray-300">
              <ReactMarkdown
                components={{
                  h1: ({ ...props }) => (
                    <h1
                      className="text-4xl font-bold text-amber-400 mb-6 pb-4 border-b-2 border-amber-500/30"
                      {...props}
                    />
                  ),
                  h2: ({ ...props }) => (
                    <h2
                      className="text-3xl font-bold text-amber-300 mb-4 mt-8"
                      {...props}
                    />
                  ),
                  h3: ({ ...props }) => (
                    <h3
                      className="text-2xl font-bold text-amber-200 mb-3 mt-6"
                      {...props}
                    />
                  ),
                  h4: ({ ...props }) => (
                    <h4
                      className="text-xl font-bold text-amber-200 mb-2 mt-4"
                      {...props}
                    />
                  ),
                  p: ({ ...props }) => (
                  ...
                  code: ({
                    ...
                    inline ? (
                      <code
                        className="bg-amber-900/30 text-amber-300 px-2 py-1 rounded text-sm font-mono"
                        {...props}
                      />
                    ) : (
                      <code
                        className="block bg-gray-800/70 text-gray-200 p-4 rounded-lg overflow-x-auto mb-4 font-mono text-sm"
                        {...props}
                      />
                    ),
                  pre: ({ ...props }) => (
                    <pre
                      className="bg-gray-800/70 rounded-lg overflow-hidden mb-4"
                      {...props}
                    />
                  ),
                  blockquote: ({ ...props }) => (
                    <blockquote
                      className="border-l-4 border-amber-500/50 pl-6 py-2 my-4 italic text-gray-400 bg-amber-900/10 rounded-r-lg"
                      {...props}
                    />
                  ),
                  strong: ({ ...props }) => (
                  ...
                  table: ({ ...props }) => (
                    <div className="overflow-x-auto mb-4">
                      <table
                        className="min-w-full border-collapse border border-amber-500/30"
                        {...props}
                      />
                    </div>
                  ),
                  thead: ({ ...props }) => (
                    <thead className="bg-amber-900/30" {...props} />
                  ),
                  th: ({ ...props }) => (
                    <th
                      className="border border-amber-500/30 px-4 py-2 text-left text-amber-300 font-bold"
                      {...props}
                    />
                  ),
                  td: ({ ...props }) => (
                    <td
                      className="border border-amber-500/30 px-4 py-2 text-gray-300"
                      {...props}
                    />
                  ),
                  hr: ({ ...props }) => (
                    <hr className="border-amber-500/30 my-8" {...props} />
                  ),
                }}
              >
                {projectDocument.function_doc}
            ...
            <div className="text-center py-16">
              <BookOpen
                size={64}
                className="mx-auto mb-4 text-amber-400 opacity-50"
              />
              <p className="text-gray-400 text-lg mb-2">
                機能要件定義書がまだ作成されていません
              </p>
              <p className="text-gray-500 text-sm">
                プロジェクトセットアップを完了してください
              </p>
            </div>
          )}
        </div>
