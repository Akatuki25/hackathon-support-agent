        ...
        <div className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 backdrop-blur-xl rounded-2xl border-2 border-pink-500/40 shadow-2xl p-8">
          {isLoading ? (
            ...
            <div className="prose prose-invert max-w-none text-gray-300">
              <ReactMarkdown
                components={{
                  h1: ({ node, ...props }) => <h1 className="text-4xl font-bold text-pink-400 mb-6 pb-4 border-b-2 border-pink-500/30" {...props} />,
                  h2: ({ node, ...props }) => <h2 className="text-3xl font-bold text-pink-300 mb-4 mt-8" {...props} />,
                  h3: ({ node, ...props }) => <h3 className="text-2xl font-bold text-pink-200 mb-3 mt-6" {...props} />,
                  h4: ({ node, ...props }) => <h4 className="text-xl font-bold text-pink-200 mb-2 mt-4" {...props} />,
                  ...
                  a: ({ node, ...props }) => <a className="text-pink-400 hover:text-pink-300 underline transition-colors" {...props} />,
                  table: ({ node, ...props }) => (
                    <div className="overflow-x-auto mb-4">
                      <table className="min-w-full border-collapse border border-pink-500/30" {...props} />
                    </div>
                  ),
                  thead: ({ node, ...props }) => <thead className="bg-pink-900/30" {...props} />,
                  th: ({ node, ...props }) => <th className="border border-pink-500/30 px-4 py-2 text-left text-pink-300 font-bold" {...props} />,
                  td: ({ node, ...props }) => <td className="border border-pink-500/30 px-4 py-2 text-gray-300" {...props} />,
                  hr: ({ node, ...props }) => <hr className="border-pink-500/30 my-8" {...props} />,
                }}
              >
                {projectDocument.specification}
              </ReactMarkdown>
