...
export default function BaseEditor({
  ...
  const [saveTimer, setSaveTimer] = useState<ReturnType<
    typeof setTimeout
  > | null>(null);
  const [internalContentInitialized, setInternalContentInitialized] =
    useState(false);

  // 内部または外部の初期化状態を使用
  const contentInitialized = isContentInitialized ?? internalContentInitialized;
  const setContentInitialized =
    onContentInitialized ?? setInternalContentInitialized;
  ...
  useEffect(() => {
    const loadContent = async () => {
      ...
      try {
        ...
        const parsedBlocks = await markdownToBlocks(
          sanitizedContent,
          editor.pmSchema,
        );
        ...
        if (parsedBlocks && parsedBlocks.length > 0) {
          ...
          console.warn(
            "Markdown解析結果が空です。プレーンテキストとして設定します。",
          );
          editor.replaceBlocks(editor.document, [
            createPlainParagraphBlock(sanitizedContent),
          ]);
        }
      } catch (error) {
        ...
        try {
          editor.replaceBlocks(editor.document, [
            createPlainParagraphBlock(content!),
          ]);
        } catch (fallbackError) {
  ...
  return (
    <div className={`flex flex-col ${containerClassName}`}>
      ...
      {/* エディターエリア */}
      <div
        className={`backdrop-blur-lg rounded-xl p-6 shadow-xl border transition-all bg-white bg-opacity-70 border-purple-500/30 shadow-purple-300/20 dark:bg-gray-800 dark:bg-opacity-70 dark:border-cyan-500/30 dark:shadow-cyan-500/20 ${className}`}
      >
        {content !== null ? (
          <>
            {/* Light mode editor */}
